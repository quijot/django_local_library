...
class AuthorCreateTest(TestCase):
    # Remember that you need to check anything that you
    # specify or that is part of the design. This will include
    # who has access,
    # the initial date,
    ...
    def test_form_date_of_death_initially_has_default_date(self):
        login = self.client.login(username="testuser2", password="2HJ1vRV0Z&3iD")
        response = self.client.get(reverse("author_create"))
        self.assertTrue(login)
        self.assertEqual(response.status_code, 200)

        default_date_of_death = "13/03/2018"
        self.assertEqual(
            response.context["form"].initial["date_of_death"], default_date_of_death
        )

    def test_redirects_to_author_detail(self):
        login = self.client.login(username="testuser2", password="2HJ1vRV0Z&3iD")
        response = self.client.post(
            reverse("author_create"), {"first_name": "Ernesto", "last_name": "Sabato"}
        )
        self.assertTrue(login)
        self.assertEqual(response.status_code, 302)

        test_author = Author.objects.filter(first_name="Ernesto")[0]
        self.assertRedirects(
            response, reverse("author-detail", kwargs={"pk": test_author.pk}),
        )
